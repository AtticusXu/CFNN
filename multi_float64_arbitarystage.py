...
if dim==2:
    ...
    t_draw_2d = np.array((x_draw_2d.reshape(N_draw_2d**2),y_draw_2d.reshape(N_draw_2d**2))).T
    t_draw_2d = tf.cast(t_draw_2d, dtype=tf.float64)
    y_draw_2d = fun_test(t_draw_2d)

    x_draw = model.predict(t_draw_2d)
    x_draw_p = x_draw
    plt.imshow(tf.reshape((y_draw_2d - x_draw_p),[N_draw_2d,N_draw_2d]), interpolation='bilinear', cmap=matplotlib.colormaps['seismic'],
...
for i in range(STAGEN-1):
    ...
    if dim==2:
        ...
        plt.imshow(tf.reshape((y_draw_2d - x_draw_p),[N_draw_2d,N_draw_2d]), interpolation='bilinear', cmap=matplotlib.colormaps['seismic'],
                   origin='lower', extent=[-1,1,-1,1])
        plt.colorbar()
        plt.savefig(plot_dir+'error_'+str(i+2)+'.png', dpi = 600, bbox_inches='tight')
